...
import { adminGetAllUsers, adminGetAllStores, adminGetAllProducts, adminGetAllOrders, GET_ALL_CATEGORIES, ADMIN_GET_DASHBOARD_STATS, GET_ALL_STORE_CATEGORIES } from '../graphql/queries';
import { ADMIN_UPDATE_USER_ROLE, ADMIN_UPDATE_USER_STATUS, ADMIN_UPDATE_CATEGORY, ADMIN_DELETE_CATEGORY, CREATE_CATEGORY, ADMIN_UPDATE_STORE_STATUS, ADMIN_UPDATE_PRODUCT_STATUS, ADMIN_CREATE_STORE_CATEGORY, ADMIN_UPDATE_STORE_CATEGORY, ADMIN_DELETE_STORE_CATEGORY } from '../graphql/mutations';
import { Container, Typography, CircularProgress, Alert, Box, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, IconButton, Menu, MenuItem, Tabs, Tab, Button, Dialog, DialogActions, DialogContent, DialogTitle, TextField, Grid, Card, CardContent } from '@mui/material';
...
const AdminDashboard = () => {
  ...
  if (usersLoading || storesLoading || productsLoading || ordersLoading || categoriesLoading || storeCategoriesLoading || statsLoading) return <Container maxWidth="lg" sx={{ mt: 4, textAlign: 'center' }}><CircularProgress /><Typography>{t('loadingAdminData')}</Typography></Container>;
  if (usersError || storesError || productsError || ordersError || categoriesError || storeCategoriesError || statsError) return <Alert severity="error">Error loading data.</Alert>;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4 }}>
      ...
      <TabPanel value={tabValue} index={5}><AnalyticsDashboard /></TabPanel>
      <Menu anchorEl={anchorElUser} open={Boolean(anchorElUser)} onClose={handleUserMenuClose}><MenuItem disabled>{t('changeRoleTo')}</MenuItem><MenuItem onClick={() => handleRoleChange('customer')}>{t('customer')}</MenuItem><MenuItem onClick={() => handleRoleChange('seller')}>{t('seller')}</MenuItem><MenuItem onClick={() => handleRoleChange('admin')}>{t('admin')}</MenuItem><MenuItem disabled>{t('changeStatusTo')}</MenuItem><MenuItem onClick={() => handleUserStatusChange('active')}>{t('active')}</MenuItem><MenuItem onClick={() => handleUserStatusChange('inactive')}>{t('inactive')}</MenuItem></Menu>
